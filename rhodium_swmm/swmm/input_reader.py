...
def read_swmm_input_file_to_dict(file_path):
    with open(file_path) as input_template_file:
        input_template = read(input_template_file)
...
def read(f):
    ...
    for line in f:
        ...
        if 'SUBCATCHMENTS' not in file_dict:
            file_dict['SUBCATCHMENTS'] = {
                'lines': [],
                'comment': None,
            }

        if 'LID_CONTROLS' not in file_dict:
            file_dict['LID_CONTROLS'] = {
                'lines': [],
                'comment': None,
            }

        if 'LID_USAGE' not in file_dict:
            file_dict['LID_USAGE'] = {
                'lines': [],
                'comment': None,
            }
